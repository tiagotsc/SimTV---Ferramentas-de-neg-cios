...
class delin_model extends faturamento_model{
    ...
    public function dadosDelin($arquivo, $link){
        ...
        $sql = "SELECT 
                ...
                Operadora,
                CNPJ,
                designacao AS 'Designacao dos Servicos',
                data_inicio AS 'Data Ativacao',
                data_fim  AS 'Data Termino',
                status AS 'contratos status',
                CONCAT('R$',format(valor,2,'de_DE')) AS 'Valor Contrato',
                CONCAT('R$',format(mens_contratada_sem_imposto,2,'de_DE')) AS 'Mensalidade',
                nome AS 'Tipo Servico',
                CONCAT('R$',format(mens_atual_sem_imposto,2,'de_DE')) AS 'Valor S/Impostos',
                CONCAT('R$',format(valor_com_impostos,2,'de_DE')) AS 'Valor C/Impostos',
                CONCAT('R$',format(taxa_inst_com_imp,2,'de_DE')) AS 'Taxa Inst (C/I)',
                faturado_por AS 'Faturado Por',            
                CONCAT('R$',format(taxa_inst_com_imp+valor_cobrado,2,'de_DE')) AS valor_cobrado,
                CONCAT('R$', format(
                	(taxa_inst_com_imp+valor_cobrado)                
                	+
                	( # Debito = devendo entao soma a mais pra pagar
                	SELECT 
                			CASE WHEN SUM(valor) IS NULL THEN 0 ELSE SUM(valor) END
                	FROM ".BANCO_TELECOM.".tcom_debcred 
                	WHERE status = 'A' AND tipo = 'D' AND faturado = 'NAO' AND tcom_debcred.idContrato = res.id
                	)
                	-
                	(
                	# Credito = esta com um extra entao diminue pra pagar
                	SELECT 
                			CASE WHEN SUM(valor) IS NULL THEN 0 ELSE SUM(valor) END
                	FROM ".BANCO_TELECOM.".tcom_debcred 
                	WHERE status = 'A' AND tipo = 'C' AND faturado = 'NAO' AND tcom_debcred.idContrato = res.id
                	)
                ,2,'de_DE')) AS valor_cobrado_deb_cred,
                CONCAT('R$', format(
                ( # Debito = devendo entao soma a mais pra pagar
                	SELECT 
                			CASE WHEN SUM(valor) IS NULL THEN 0 ELSE SUM(valor) END
                ...
                SELECT
                    ...
                    tcom_contrato.id,
                    filhaOper.titulo AS Operadora,                    
                    tcom_oper_cobr.cnpj AS CNPJ,
                    tcom_circuito.designacao,
                    tcom_contrato.status,
                    data_inicio,
                    data_fim,
                    tcom_contrato_valor.valor,
                    tcom_contrato_valor.mens_contratada_sem_imposto,
                    tcom_servico.nome,                    
                    tcom_contrato_valor.mens_atual_sem_imposto,
                    ".BANCO_TELECOM.".valorComImposto(tcom_contrato_valor.mens_atual_sem_imposto,isencao_icms,idServico,tcom_contrato.cd_unidade,tcom_contrato_valor.faturado_por, filhaOper.estatal) AS 'valor_com_impostos',
                    CASE 
                        WHEN SUBSTR(tcom_contrato.data_inicio,1,7) = '".$data."' AND tcom_contrato_valor.gerouInst = 'N'
                    ...
                    CASE 
                    	WHEN DATEDIFF(data_canc, '".$data."-01') > 0 AND SUBSTR(data_canc,1,7) = '".$data."' # TRATAMENTO DATA DE CANCELAMENTO
                            ...
                            THEN ROUND((".BANCO_TELECOM.".valorComImposto(tcom_contrato_valor.mens_atual_sem_imposto,isencao_icms,idServico,tcom_contrato.cd_unidade,tcom_contrato_valor.faturado_por, filhaOper.estatal) / 30) * ".BANCO_TELECOM.".diasCancelFaturar(data_canc,'".$data."-01'),2)
                        WHEN DATEDIFF(tcom_contrato.data_fim, '".$data."-01') > 0 AND SUBSTR(tcom_contrato.data_fim,1,7) = '".$data."' AND tcom_contrato.status = 'A' # TRATAMENTO DATA DE TERMINO ATIVO
                    		THEN ROUND(".BANCO_TELECOM.".valorComImposto(tcom_contrato_valor.mens_atual_sem_imposto,isencao_icms,idServico,tcom_contrato.cd_unidade,tcom_contrato_valor.faturado_por, filhaOper.estatal),2)
                        WHEN DATEDIFF(tcom_contrato.data_fim, '".$data."-01') > 0 AND SUBSTR(tcom_contrato.data_fim,1,7) = '".$data."' AND tcom_contrato.status = 'I' # TRATAMENTO DATA DE TERMINO INATIVO
			            ...
			            WHEN DATEDIFF(LAST_DAY('".$data."-01'), tcom_contrato.data_inicio) >= 0 AND SUBSTR(tcom_contrato.data_inicio,1,7) = '".$data."' # TRATAMENTO DATA DE ATIVAÇÃO
                            ...
                            THEN ROUND((".BANCO_TELECOM.".valorComImposto(tcom_contrato_valor.mens_atual_sem_imposto,isencao_icms,idServico,tcom_contrato.cd_unidade,tcom_contrato_valor.faturado_por, filhaOper.estatal) / 30) * ".BANCO_TELECOM.".diasAtivacaoFaturar(tcom_contrato.numero,'".$data."-01', tcom_contrato.data_inicio),2)
                    ...
                    (SELECT nome FROM adminti.unidade WHERE cd_unidade = faturado_por) AS 'faturado_por'
                ...
                AND (
                    ...
                    OR (SUBSTR(data_inicio,1,7) <= '".$data."' AND data_ult_fatura IS NULL AND tcom_contrato.status = 'A')
                    )
    ...
    public function gravaDelin($arquivo, $link){
        ...
        $sql = "SELECT 
        		...
        		( # Debito = devendo entao soma a mais pra pagar
        				SELECT 
        						CASE WHEN SUM(valor) IS NULL THEN 0 ELSE SUM(valor) END
        				FROM ".BANCO_TELECOM.".tcom_debcred 
        				WHERE status = 'A' AND tipo = 'D' AND faturado = 'NAO' AND tcom_debcred.idContrato = res.idContrato
        		)
        		-
        		(
        				# Credito = esta com um extra entao diminue pra pagar
        				SELECT 
        						CASE WHEN SUM(valor) IS NULL THEN 0 ELSE SUM(valor) END
        				FROM ".BANCO_TELECOM.".tcom_debcred 
        				WHERE status = 'A' AND tipo = 'C' AND faturado = 'NAO' AND tcom_debcred.idContrato = res.idContrato
        		) AS valor_cobrado,
                ( # Debito = devendo entao soma a mais pra pagar
        				SELECT 
        						CASE WHEN SUM(valor) IS NULL THEN 0 ELSE SUM(valor) END
                ...
                FROM (
                    SELECT
                        ...
                        CASE 
                            WHEN SUBSTR(tcom_contrato.data_inicio,1,7) = '".$data."'
                                THEN ".BANCO_TELECOM.".valorComImposto(tcom_contrato_valor.taxa_inst_sem_imposto,isencao_icms,CASE WHEN tcom_contrato.idServico = 6 THEN 6 ELSE 11 END,tcom_contrato.cd_unidade,faturado_por,filhaOper.estatal) 
                        ELSE '0.00' END AS taxa_inst_com_imposto,
        ...
        $dtComp = implode('-',array_reverse(explode('/',$this->input->post('data'))));
        foreach($resultado as $res){
            ...
            $campo[] = 'valor_debito';
            $valor[] = $this->util->formaValorBanco($res->valor_debito);
            $campo[] = 'valor_credito';
            $valor[] = $this->util->formaValorBanco($res->valor_credito);
            $campo[] = 'idCredito';
            $valor[] = $this->util->formaValorBanco($res->idCredito);
            $campo[] = 'idDebito';
            $valor[] = $this->util->formaValorBanco($res->idDebito);
            ...
            if($res->gerouInst == 'N' and $data == substr($res->data_inicio,0,7)){ 
    ...
    public function dadosDelinTabelaDinamica(){
        ...
        $sql = "SELECT
                	idPai, 
                	(
                	SELECT COUNT(*) FROM ".BANCO_TELECOM.".tcom_delin WHERE CASE WHEN idPai = '' THEN idOperCobrPai IS NULL ELSE idOperCobrPai = idPai END AND competencia = DATE_ADD('".$data."-01',INTERVAL 1 MONTH)
                	) AS existe,
                	(
                	SELECT COUNT(*) FROM ".BANCO_TELECOM.".tcom_delin WHERE CASE WHEN idPai = '' THEN idOperCobrPai IS NULL ELSE idOperCobrPai = idPai END AND competencia = DATE_ADD('".$data."-01',INTERVAL 1 MONTH) AND status = 'FATURADO'
                    ...
                    (
                		SELECT DISTINCT arquivo FROM ".BANCO_TELECOM.".tcom_delin WHERE CASE WHEN idPai = '' THEN idOperCobrPai IS NULL ELSE idOperCobrPai = idPai END ORDER BY tcom_delin.data_cadastro DESC LIMIT 1
                	) AS arquivo,
                	grupo, 
                    SUM(tot) AS qtdOper,
                    ...
                    ROUND(SUM(valor_cobrado_inst),2) AS valor_cobrado_inst
                FROM (
                	SELECT
 ...
 AS valor_cobrado_inst
                    ...
                    AND SUBSTR(data_inicio,1,7) = '".$data."'
                	/*AND (
                        (SUBSTR(data_inicio,1,7) <= '".$data."' AND SUBSTR(data_fim,1,7) >= '".$data."' AND tcom_contrato.status = 'A')
                	OR (SUBSTR(data_canc,1,7) >= '".$data."' AND tcom_contrato.status = 'I')
                    OR (SUBSTR(data_inicio,1,7) <= '".$data."' AND data_ult_fatura IS NULL AND tcom_contrato.status = 'A')
    ...
    public function delinArquivos(){
        ...
        $this->db->order_by('tcom_delin.data_cadastro', 'desc');
        return $this->db->get(BANCO_TELECOM.'.tcom_delin')->result();
