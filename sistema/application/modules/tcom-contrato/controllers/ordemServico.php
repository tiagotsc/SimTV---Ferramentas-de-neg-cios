...
class OrdemServico extends Base {
     
     private $logDados;
     const modulo = 'tcom-contrato';
     const controller = 'ordem';
     const pastaView = 'ordem';
     const tabela = 'tcom_ordem';
     const assunto = 'Ordem';
     const modelAssunto = 'ordem';
     const perModulo = 274;
     const perPesq = 326;
     const perImprimir = 328;
     const perAnexar = 330;
     const perVisualizarHistorico = 337; # O mesmo do controller "viabilidadeResp"
     const perDesignacao = 414;
     const perAtivacao = 349;
     const perNumero = 418;
     const perVigencia = 415;
     const perServico = 427;
     const perStatus = 329;
     const perPontaB = 419;
     const perCadEdit = 413;
     const perDeletar = 331;
     const perValores = 407;
     const emailEnviaEnviaAnalise = 56;
     const emailEnviaRecebeRespostaAnalise = 57;
    ...
    public function __construct(){
        ...
        $this->load->model('tcom-contrato/ordem_model',self::modelAssunto);
    ...
    public function geraOrdem($idContrato){
        $_POST['idViab'] = 'null';
        $_POST['idTipo'] = 6;
        $_POST['idViabResp'] = 'null';
        $status = $this->ordem->geraOrdem($idContrato);
        
        if($status){
            $this->session->set_flashdata('statusOperacao', '<div class="alert alert-success"><strong>Ordem gerada com sucesso!</strong></div>');
        }else{
            $this->session->set_flashdata('statusOperacao', '<div class="alert alert-danger">Erro ao gerar ordem, caso o erro persiste comunique o administrador!</div>');
        }
        
        redirect(base_url('/tcom-contrato/contrato/ficha/'.$idContrato));
    }
    
    public function pdf($idCampo = 2, $campo = 'tcom_ordem.id'){
        
        $arquivo = $this->conteudo($idCampo, $campo);
        
        $parametros = "'en-x','A4','','',5,5,5,5,5,5,'P'";
        //load mPDF library
		$this->load->library('m_pdf',$parametros);
    ...
    public function conteudo($idCampo = 2, $campo = 'tcom_ordem.id'){
        
        $modelAssunto = self::modelAssunto;
        
        $dados['ordem'] = $this->$modelAssunto->dadosOrdem($idCampo, $campo);
        $dados['contrato'] = $this->crudModel->dadosIDSimples(BANCO_TELECOM.'.tcom_contrato','id',$dados['ordem']->idContrato);
        $contratoCircuito = $this->crudModel->dadosIDSimples(BANCO_TELECOM.'.tcom_contrato_circuito','idContrato',$dados['contrato']['id']);
        
        $dados['circuito'] = $this->contrato->contratoCircuito($dados['ordem']->idContrato);
        ...
        $dados['operadoraEstado'] = $this->dadosBanco->estado($dados['operadoraInst']['cd_estado']);
        #echo '<pre>'; print_r($dados['operadoraInst']); exit();
        ...
        $dados['clienteEstado'] = $this->dadosBanco->estado($dados['clienteEnd']['cd_estado']);
        $dados['clienteEndTel'] = $this->cliente->telefones($dados['cliente']['id']);
        #echo '<pre>'; print_r(array_column($dados['clienteEndTel'] ); exit();
        /*$dados['contratoDados'] = $contratoCircuito;
        
        $dados['unidade'] = $this->crudModel->dadosIDSimples('adminti.unidade','cd_unidade',$dados['contrato']['cd_unidade']);
        $dados['interface'] = $this->crudModel->dadosIDSimples(BANCO_TELECOM.'.tcom_interface','id',$contratoCircuito['idInterface']);
        $dados['taxa_digital'] = $this->crudModel->dadosIDSimples(BANCO_TELECOM.'.tcom_taxa_digital','id',$contratoCircuito['idTaxaDigital']);*/
        
        return $this->load->view(self::pastaView.'/view_imprimir', $dados, true);
    ...
    public function ficha($id = false){
        ...
        $dados['perServico'] = self::perServico;
        ...
        $dados['servicosTipo'] = $this->contrato->servicosTipos();
        $dados['servicos'] = $this->contrato->servicos($dados['idServico']);   
    ...
    */
    ...
    /*public function anexaArquivo(){
        ...
        if($_FILES['anexo']['tmp_name']){
            ...
            $file_name = $this->input->post('anexo_nome');
    ...
    public function alteraServico(){
        ...
        try{
            $status = $this->$modelAssunto->alterarServicos();
            
            $this->logDados['descricao'] = ucfirst(self::modulo).' - '.self::assunto.' - Altera serviços '.self::assunto;
            $this->logDados['acao'] = 'UPDATE'; 
            $this->logDados['idAcao'] = $this->input->post('id');
        ...
        if($status){
            $this->session->set_flashdata('statusOperacao', '<div class="alert alert-success"><strong>Servi&ccedil;os do '.strtolower(self::assunto).' alterado com sucesso!</strong></div>');
        }else{
            $this->session->set_flashdata('statusOperacao', '<div class="alert alert-danger">Erro ao alterar servi&ccedil;os do '.strtolower(self::assunto).', caso o erro persiste comunique o administrador!</div>');
        }
        redirect(base_url(self::modulo.'/'.self::controller.'/ficha/'.$this->input->post('id')));
