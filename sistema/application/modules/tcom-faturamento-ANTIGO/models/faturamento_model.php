    ...
    public function dadosTabelaDinamica(){
        ...
        if($this->input->post('tipo_acao') == 'faturamento'){
            ...
            $sql = "SELECT
                    	...
                    	'-' AS existe,
                    	(
                    	SELECT COUNT(*) FROM ".BANCO_TELECOM.".tcom_delin WHERE CASE WHEN tcom_oper.id IS NULL THEN idOperCobrPai IS NULL ELSE idOperCobrPai = tcom_oper.id END AND status = 'FATURADO'
                    	) AS faturado,
                    	(
