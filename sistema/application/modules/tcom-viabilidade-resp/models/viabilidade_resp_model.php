...
class Viabilidade_resp_model extends CI_Model{
    ...
    public function pesquisa($parametros, $mostra_por_pagina, $sort_by, $sort_order, $pagina){
        ...
        */
        ...
        $this->db->select("
                            ...
                            ELSE '' END AS aprovacao,
                            CASE WHEN tcom_status_hist.nome IS NULL THEN 'Vistoriado' ELSE tcom_status_hist.nome END AS status,
                            DATE_FORMAT(prazo_ativacao,'%d/%m/%Y') AS prazo,
                            DATE_FORMAT(tcom_viab_resp.data_cadastro,'%d/%m/%Y %H:%i:%s') AS data_cadastro_resp,
                            CASE 
        ...
        if($sort_by != '1'){
            ...
            $campoOrder['data_cadastro_resp'] = 'tcom_viab_resp.data_cadastro';
            $sort_by = (isset($campoOrder[$sort_by])) ? $campoOrder[$sort_by]: $sort_by;
        ...
        $dados['camposLabel'] = array('n_solicitacao' => 'Nº pedido', 'numero' => 'Nº contrato', 'data_cadastro_resp' => 'dataCad');
        $dados['campos'] = array('id', 'n_solicitacao', 'numero', 'designacao', 'tipo', 'viavel', 'data_cadastro_resp', 'aprovacao', 'status');
    ...
    public function dadosViabRespPorViabilidade($id = false){
        ...
        $this->db->select("
                            tcom_viab_resp.id,
                            tcom_viab_resp.idViab,
                            tcom_viab_resp.cd_unidade,
                            tcom_viab_resp.viavel,
                            tcom_viab_resp.end_enco,
                            tcom_viab_resp.anexo,
                            tcom_viab_resp.cabo,
                            tcom_viab_resp.cordoalha,
                            tcom_viab_resp.canalizacao,
                            tcom_viab_resp.node_distancia,
                            tcom_viab_resp.aprovacao,
                            tcom_viab_resp.data_aprovacao,
                            tcom_viab_resp.cd_usuario_aprovacao,
                            tcom_viab_resp.observacao,
                            tcom_viab_resp.cd_usuario,
                            tcom_viab_resp.idContrato,
                            tcom_viab_resp.idContratoAtual,
                            tcom_viab_resp.gerou_contrato,
                            tcom_viab_resp.idStatusHist,
                            tcom_viab_resp.ativou,
                            tcom_viab_resp.data_ativacao,
                            tcom_viab_resp.cd_usuario_ativacao,
                            tcom_viab_resp.data_cadastro,
                            tcom_contrato.numero,
    ...
    public function atualizaAprovacao(){
        ...
        if($this->input->post('aprovacao') == 'S'){
            if($this->input->post('idTipo') == 1 or $this->input->post('idTipo') == 7){ # Ativação, Co-Location
            ...
            if($this->input->post('idTipo') == 1  or $this->input->post('idTipo') == 7 /*and trim($this->input->post('nome_cliente')) != ''*/){
                $this->atualizaNomeCliente();
            }
            
            if(in_array($this->input->post('idTipo'),array(2,3,5,6))){ # UPGRADE, DOWNGRADE, MUDANÇA DE ENDEREÇO, OUTRAS MUDANÇAS
        ...
        if($contratoGerado){ // Se aprovação gerou contrato
            ...
            if(in_array($this->input->post('idTipo'),array(2,3,5,6))){
