...
class circuito extends Base {
     
     private $logDados;
     const modulo = 'tcom-circuito';
     const controller = 'circuito';
     const pastaView = 'circuito';
     const tabela = 'tcom_circuito';
     const assunto = 'Circuito';
     const modelAssunto = 'circuito';
     const perModulo = 274;
     const perPesq = 310;
    ...
    public function __construct(){
        ...
        $this->load->model('circuito_model',self::modelAssunto);
    ...
    public function alterarCircuito(){
        
        $modelAssunto = self::modelAssunto;
        
        try{
            $status = $this->$modelAssunto->atualizaInterfaceVelocidade();
            
            $this->logDados['descricao'] = ucfirst(self::modulo).' - '.self::assunto.' - Altera dados circuito '.self::assunto;
            $this->logDados['acao'] = 'UPDATE'; 
            $this->logDados['idAcao'] = $this->input->post('idCircuito');
        ...
        if($status){
            $this->session->set_flashdata('statusOperacao', '<div class="alert alert-success"><strong>Dados do '.strtolower(self::assunto).' alterado com sucesso!</strong></div>');
        }else{
            $this->session->set_flashdata('statusOperacao', '<div class="alert alert-danger">Erro ao alterar dados do '.strtolower(self::assunto).', caso o erro persiste comunique o administrador!</div>');
        }
        redirect(base_url('tcom-contrato/contrato/ficha/'.$this->input->post('idContrato')));
