                    ...
                    <?php if(in_array(6, $dashboardPermitidos)){ ?>
                          ...
                          <div>
                            ...
                            <div class="row dashCamPreechimento">
                                ...
                                <?php
                                $options = array('' => '');		
                        		foreach($mesAnoStatusBoleto as $mSb){
                        			$options[$mSb->VALOR] = $mSb->EXIBICAO;
                        		}
                        		
                        		echo form_label('Selecione o m&ecirc;s/ano:', 'mes_ano_status_boleto');
                        		echo form_dropdown('mes_ano_status_boleto', $options, '', 'id="mes_ano_status_boleto" class="form-control"');
                                ?>
                                </div>
                            </div>
                            <div id="div_status_titulo">
                                <h5 class="ocultar tituloGrafico">Informa os valores para todos os ciclos</h5>
                                <div class="ocultar" id="chart4"></div>
                                <div class="row dashCamPreechimento ocultar">
                                    <div id="div_ciclos" class="col-md-6">
                                    </div>
                                </div>
                                <h5 class="ocultar tituloGrafico">Informa a quantidade por banco de determinado ciclo</h5>
                                <div>
                                    <div class="ocultar" id="chart5"></div>
                                </div>
                            </div>
                          </div>
                    <?php } ?>
...
$(document).ready(function(){
    ...
    $("#mes_ano_status_boleto").change(function(){         
        ...
        if($(this).val() != ""){
            ...
            $(this).graficoBarraColadaStatusTitulo();
            
            $.ajax({
              type: "POST",
              url: '<?php echo base_url(); ?>ajax/comboCicloStatusBoleto/'+$(this).val(),
              dataType: "json",
              error: function(res) {
              ...
              success: function(res) {
               ...
               var select = '<label for="ciclo_status_boleto">Selecione o ciclo:</label>';
                   select += '<select name="ciclo_status_boleto" id="ciclo_status_boleto" class="form-control">';
                ...
                $("#ciclo_status_boleto").change(function(){ 
        
                    if($(this).val() != ""){
                        $(this).graficoBarraColadaCicloStatusTitulo();
                    }else{
                        $('#chart5').html('');
                    }
...
/*################### STATUS DOS TÍTULOS - INÍCIO #######################*/

$.fn.graficoBarraColadaStatusTitulo = function() {
    // GRÁFICO BARRA COLADA            
    c3.generate({
        bindto: '#chart4',
        data: {
            //url: '<?php echo base_url('assets/c3_test.json')?>',
            url: '<?php echo base_url(); ?>ajax/statusBoletosMeses/'+$('#mes_ano_status_boleto').val(),
            mimeType: 'json',
            type: 'bar',
            x : 'ciclos',
            columns: ['ciclos','Pago', 'Pago Antes', 'Enviado', 'Rejeitado', 'Não retornado'/*, 'Não gerado'*/],
            groups: [
...
$.fn.graficoBarraColadaCicloStatusTitulo = function() {
    ...
    c3.generate({
        ...
        data: {
            //url: '<?php echo base_url('assets/c3_test.json')?>',
            url: '<?php echo base_url(); ?>ajax/statusBoletosCiclos/'+$('#mes_ano_status_boleto').val()+'/'+$("#ciclo_status_boleto").val(),
            mimeType: 'json',
            type: 'bar',
            x : 'bancos',
            columns: ['bancos','Pago', 'Pago Antes', 'Registrado', 'Rejeitado', 'Não retornado'/*, 'Não gerado'*/],
            groups: [
