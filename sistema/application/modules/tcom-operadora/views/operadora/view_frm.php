                    	...
                    	echo form_open($modulo.'/'.$controller.'/salvar',$data);
                    		...
                    		echo form_fieldset("Ficha ".$assunto."<a href='".base_url($modulo.'/'.$controller.'/pesq')."' class='linkDireita'><span class='glyphicon glyphicon-arrow-left'></span>&nbspVoltar Pesquisar</a>", $attributes);
                                ...
                                echo '<div class="row">';
                                    ...
                                    echo '<div id="tipoProtecao"></div>';
                                    echo form_label('Tipo', 'tipo');
                                    $options = array('PEDIDO' => 'Pedido', 'FATURAMENTO' => 'Faturamento');		
                            		echo form_dropdown('tipo', $options, $tipo, 'id="tipo" class="form-control"');
                                    echo '</div>'; 
                                    
                                    echo '<div class="col-md-2 fat">';
                                    echo form_label('Estatal', 'estatal');
                                    $options = array('SIM' => 'Sim', 'NAO' => 'Não');		
                            		echo form_dropdown('estatal', $options, $estatal, 'id="estatal" class="form-control"');
                                    echo '</div>';
                                ...
                                echo '<div class="row fat">';
                                    echo '<div class="col-md-12 cabecalhoDivisor text-center"><strong>DADOS FATURAMENTO</strong></div>';
                                echo '</div>';
                                ...
                                echo '<div class="row fat">';
                                    ...
                                    echo '<div class="col-md-2">';
                                    $options = array('' => '');		
                                    foreach($indiceReajuste as $indResj){
                                        $options[$indResj->id] = $indResj->nome;
                                    }		
                                    ...
                                    echo '<div id="div_mes_reajuste" class="col-md-2">';
                                    $options = array('' => '');	
                                    for($i = 1; $i <=12; $i++){
                                        $options[$i] = $i;
                                    }		
                            		echo form_label('M&ecirc;s reajuste<span class="obrigatorio">*</span>', 'mes_reajuste');
                            		echo form_dropdown('mes_reajuste', $options, $mes_reajuste, 'id="mes_reajuste" class="form-control"');
                                    echo '</div>';
                                    
                                    echo '<div class="col-md-2">';
                                    echo form_label('Multa %', 'multa_porc');
                        			$data = array('name'=>'multa_porc', 'value'=>$multa_porc,'id'=>'multa_porc', 'class'=>'form-control porcentagem');
                        			echo form_input($data);
                                    echo '</div>';
                                ...
                                echo '<div class="row">';
                        			...
                        			$data = array('name'=>'endereco', 'value'=>$endereco,'id'=>'endereco', 'placeholder'=>'Informe o endereço', 'class'=>'form-control');
                        			echo form_input($data);
                                    echo '</div>';
                        			...
                        			$data = array('name'=>'bairro', 'value'=>$bairro,'id'=>'bairro', 'placeholder'=>'Informe o bairro', 'class'=>'form-control');
                        			echo form_input($data);
                                    echo '</div>';
                                ...
                                echo '<div class="row">';
                            		...
                            		echo form_dropdown('cd_estado_cob', $options, $cd_estado_cob, 'id="cd_estado_cob" class="form-control"');
                                    echo '</div>';  
                                    
                                    echo '<div class="col-md-5">';
                                    echo form_label('Endereço<span class="obrigatorio obriOpcional">*</span>', 'endereco_cob');
                        			$data = array('name'=>'endereco_cob', 'value'=>$endereco_cob,'id'=>'endereco_cob', 'placeholder'=>'Informe o endereço', 'class'=>'form-control');
                        			echo form_input($data);
                                    echo '</div>';
                        			...
                        			$data = array('name'=>'bairro_cob', 'value'=>$bairro_cob,'id'=>'bairro_cob', 'placeholder'=>'Informe o bairro', 'class'=>'form-control');
                        			echo form_input($data);
                                    echo '</div>';
...
if($("#idRegraReajuste").val() == 1){
    $("#div_mes_reajuste").show();
}else{
    $("#div_mes_reajuste").hide();
}

$("#idRegraReajuste").change(function(){
    if($(this).val() == 1){ // Anual
        $("#div_mes_reajuste").show();
        
        $( "#mes_reajuste" ).rules( "add", {
          required: true,
          messages: {
        ...
        $( "#mes_reajuste" ).rules( "remove");
    }
})

$('.porcentagem').mask('000');

if($("#tipo").val() == 'FATURAMENTO'){
    $(".fat").show();
}else{
    $(".fat").hide();
}
...
$("#pai").change(function(){
    ...
    if($(this).val() != '0'){
        ...
        $("#tipo option[value='']").remove();
        $("#tipoProtecao").removeClass("maskProt");
        ...
        $("#tipo").val('');
        $("#tipoProtecao").addClass("maskProt");
    ...
    if($(this).val().length == 9){ // CEP Completo
        ...
        $.ajax({
              ...
              success: function(res) {
                $("#endereco"+auxNome).val(res.logradouro);
...
/*$.get( "http://api.postmon.com.br/v1/cep/20931785", function( data ) {
  dump(data);
}, "json" );*/
