...
class Ordem_model extends CI_Model{
    ...
    public function proximaOrdem(){
        ...
        $this->db->like('numero', $mesSigla, 'after'); 
        $qtd = $this->db->count_all_results(BANCO_TELECOM.'.'.self::tabela);
        return $mesSigla.'-'.($qtd + 1);
    }
    
    public function geraOrdem($idContrato){
        
        $sql = "INSERT INTO ".BANCO_TELECOM.".".self::tabela."(numero,idViab,idViabTipo,idViabResp,idContrato) ";
        $sql .= "VALUES('".$this->proximaOrdem()."', ".$this->input->post('idViab').", ".$this->input->post('idTipo').", ".$this->input->post('idViabResp').", ".$idContrato.")";
        return $this->db->query($sql);
        
    }
    
    public function dadosOrdem($idCampo, $campo){
        
        $this->db->select("
        tcom_ordem.numero,
        tcom_viab_tipo.nome AS viab_tipo,
        tcom_ordem.idContrato,
        DATE_FORMAT(data_cadastro,'%d/%m/%Y %H:%i:%s') AS data_cadastro,
        DATE_FORMAT(data_conclusao,'%d/%m/%Y %H:%i:%s') AS data_conclusao,
        tcom_ordem.status 
        ");
        $this->db->join(BANCO_TELECOM.'.tcom_viab_tipo', 'tcom_viab_tipo.id = tcom_ordem.idViabTipo');
        $this->db->where($campo, $idCampo);
        return $this->db->get(BANCO_TELECOM.'.'.self::tabela)->row();
        
    }
    
    public function daBaixaOrdem($id, $tipoId, $status){
        $sql = "UPDATE ".BANCO_TELECOM.".tcom_ordem SET status = '".$status."', data_conclusao='".date('Y-m-d H:i:s')."' WHERE ".$tipoId." = ".$id;
        $this->db->query($sql);
    }
    ...
    public function pesquisa($parametros, $mostra_por_pagina, $sort_by, $sort_order, $pagina){
        ...
        if($parametros){
            ...
            foreach($post as $campoValor){
                ...
                if($res[1] != ''){
                    ...
                    }elseif(in_array($res[0], array('viab_tipo'))){
                        $this->db->where('tcom_viab_tipo.nome', $res[1]);
                    }else{
        ...
        $dados['campos'] = array('id', 'numero', 'viab_tipo', 'data_cadastro', 'data_conclusao', 'status');
    ...
    public function execAtivacao(){
        ...
        $sql = "UPDATE ".BANCO_TELECOM.".tcom_contrato SET data_inicio = CURDATE(), cd_usuario_atualizacao = '".$this->session->userdata('cd')."', status = 'A' WHERE id = ".$this->input->post('idContrato');
        $this->db->query($sql);
    ...
    public function valoresContrato($id){
        ...
        $this->db->select("
                            ...
                            DATE_FORMAT(data_ult_fatura,'%m/%Y') AS data_ult_fatura,
		...
		foreach($_POST as $c => $v){
            ...
            if(!in_array($c, array('nome_contrato_valor','valor_id', 'email'))){
                if($this->input->post($c) != ''){
                        ...
                        if(in_array($c, array('data_pri_fatura', 'data_ult_fatura'))){
                            $_POST[$c] = '01/'.$_POST[$c];
                        }
