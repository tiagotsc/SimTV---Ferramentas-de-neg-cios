...
class tbase_model extends CI_Model {
    ...
    function test() {
        ...
        return $this->con->get('xservice');
    }

    function mountUserObj() {
        if (!$this->input->is_cli_request() && $this->session->userdata('login')) {
            $login = $this->session->userdata('login');
            $this->user = $this->con->select('num_id,permissions,home')->like('login', $login)->get('user')->row_array();
//            $this->user = $con->select('num_id,permissions,home')->get_where('user', array('login' => '%' . $login . '%'))->row_array();
            if (!$this->user) {
    ...
    function basicAuth() {
        $page = $this->con->where('name', $this->router->class)->get('page')->row_array();
        if ($page) {
            $this->ntitle = $page['descr'];
            switch ($page['auth_mode']) {
                case 'auto':
                    $page_perms = $con->where('page', $this->router->class)->get('page_perms')->result_array();
                    $e = 0;
    ...
    function _dash_utils() {
        ...
        $sys = $this->con->select('name,value')->get('system_args')->result_array();
        $args = array();
        ...
        if ($this->router->class === 'dashboard' || $this->router->class === 'adm' || $this->input->is_cli_request()||true
        ) {
//            echo 1;exit();
            $lm = $this->con->order_by('m', 'desc')->limit(1)->get('mes')->row_array();
            ...
            $a = $this->con->order_by('id', 'asc')->get('area')->result_array();
...
//        echo var_dump($this->sim_imb_meta);exit();
    }

    function user_prefs() {
        $prefz = array();
        $prefs = $this->con->where('user', $this->user['login'])->get('user_pref')->result_array();
        foreach ($prefs as $p)
    ...
    function notificar_fidelizacao($update) {
        $a = $con->get_where('acomp', array('id' => $update['parent']));
        if ($a) {
            $a = $a->row_array();
            $ass = $con->
    ...
    function notificacoes_de_assinante() {
        ...
        foreach ($asses as $a) {
            $notif = $con->order_by('date', 'asc')->limit(1)->get_where('ass_notif', array('ass' => $a['ass'], 'per' => $a['per']))->row_array();
    ...
    function checa_quarentena() {
        $acomps = $con->where('status', 2)->get('acomp')->result_array();
        foreach ($acomps as $a) {
            $reborn = $con->query(
                            "SELECT
					COUNT(*) as c
            ...
            if (intval($reborn['c']) > 0) {
                $con->where('id', $a['id'])->update('acomp', array('status' => 0, 'stage' => 1, 'solved_in' => null));
            }
        }
        return true;
    }
    ...
    function salvar_parametros_do_sistema($args) {
        foreach ($args as $key => $val) {
            $con->where('name', $key)->update('system_args', array('value' => $val));
        }
    }

    function usuario_fidelizacao($user) {
        return ($this->con->where('user', $user)->get('fid')->num_rows() > 0);
    }

    function areas_coordenadas_por_usuario($user) {
        $gs = $this->con->select('COUNT(*) as c')->where('user', $user)->get('global_coord')->row_array();
        if ($gs && intval($gs['c']) > 0) {
            return true;
        } else {
            $a = $this->con->select('area')->where('user', $user)->get('area_coord')->result_array();
            if ($a) {
                $areas = array();
                foreach ($a as $b) {
                    $areas[] = intval($b['area']);
    ...
    function historico_imb_por_produto() {
        $magic = "SELECT mm as m,a.id as areaID,a.name as area,
				(
					SELECT COUNT(*)
					FROM repair
			 ...
			 JOIN area a
			 ORDER by areaID,m";
        $r = $con->query($magic)->result_array();
        $magic = "SELECT mm as m,10 as areaID,'SIM' as area,
				(
					SELECT COUNT(*)
					FROM repair
			 ...
			 FROM mes
			 ORDER by m";
        $r = array_merge($r, $con->query($magic)->result_array());
        return $r;
    }
    ...
    function sim_bases() {
		...
		ORDER BY cservice";
        return $con->query($magic)->result_array();
    }
    ...
    function area_name($area) {
        ...
        if (is_numeric($area)) {
            ...
            $con->where('name', $area)->or_where('name_abbr', $area);
        }

        $myarea = $con->get('area')->row_array();
    ...
    function area_abbr($area) {
        ...
        $this->con->select('name_abbr a');

        if (is_numeric($area)) {
            $this->con->where('id', $area);
        } else {
            $this->con->where('name', $area)->or_where('name_abbr', $area);
        }

        $myarea = $this->con->get('area')->row_array();
        return (($myarea) ? $myarea['a'] : null);
    }
    ...
    function _rcount($a, $m, $p) {
        $this->con->select('COUNT(*) as c');
        if ($a)
            $this->con->where('area', $a);

        if ($m)
            $this->con->where('month', $m . '-01');

        if ($p)
            $this->con->where('xservice', $p);

        $r = $this->con->get('repair')->row_array();
        return intval($r['c']);
    }
    ...
    function _basecount($a, $m, $p) {
        $this->con->select('SUM(base) as base');
        ...
        if ($m) {
            ...
            $this->con->where('month', $this->currmonth);
        }

        if ($p) {
            if ($p === 'cm')
                $this->con->where('(cservice = 1 OR cservice = 2)', null, false);
            if ($p === 'tv')
                $this->con->where('(cservice = 2 OR cservice = 3)', null, false);

            if ($p === 'cb')
                $this->con->where('cservice', 2);
            if ($p === 'tv0')
                $this->con->where('cservice', 3);
            if ($p === 'cm0')
                $this->con->where('cservice', 1);
        }

        $b = $this->con->get('cbase')->row_array();
    ...
    function mmeta($meta, $a, $d = false) {
        ...
        $this->con->select('value');
        $this->con->where('name', $meta);
        $this->con->where('area', $a);

        if ($d)
            $this->con->where("date_format(ini,'%Y-%m') <=", $d);

        $m = $this->con->order_by('ini DESC, id DESC')->limit(1)->get('area_meta')->row_array();
    ...
    function mes_fechado($mes = false) {
        ...
        return ( $mes < date('Y-m') || (date('d', strtotime($this->dia_da_ultima_reclamacao())) === date('j', strtotime($this->currmonth))) );
    }
    ...
    function dia_da_primeira_reclamacao($area = false) {
        $con->select_min('repair.date', 'fdoh');
        if ($area && $area !== 'SIM')
            $con->where('repair.area', $this->area_id($area));
        $r = $con->get('repair');
        if ($r) {
            $a = $r->row_array();
            return $a['fdoh'];
    ...
    function dia_da_ultima_reclamacao($area = false) {
        $this->con->select_max('repair.date', 'ldoh');
        if ($area && $area !== 'SIM')
            $this->con->where('repair.area', $this->area_id($area));
        $r = $this->con->get('repair');
        if ($r) {
            $a = $r->row_array();
            return $a['ldoh'];
        } else
    ...
    function lista_meses($ord = 'DESC') {
        return $con->select('mm as month')->order_by('mm', $ord)->get('mes')->result_array();
    }
    ...
    function busca_area_metas($id, $time = false) {
        ...
        $a = $con->select('color')->get_where('area', array('id' => $id))->row_array();
        $metas['color'] = $a['color'];
        return $metas;
    }

    function lista_acomp_status() {
        return $con->select('id,name')->order_by('id', 'asc')->get('acomp_status')->result_array();
    }

    function lista_acomp_stages() {
        return $con->select('id,name')->order_by('id', 'asc')->get('acomp_stage')->result_array();
    }

    function lista_areas() {
        return $con->select('area.id as id,area.name as name,area.name_abbr')->order_by('name', 'asc')->get('area')->result_array();
    }
    ...
    function timeline_reclamacoes($m, $area = false) {
        ...
        $pers = null;
        if ($areaID) {
            $pers = $con->select('abbr')->where('area', $areaID)->get('per')->result_array();
            $pers = array_map(function($per) {
                return $per['abbr'];
            }, $pers);
        }
    ...
    function cadInstFilter($mes = null, $prod = false) {
        ...
        if ($mes) {
            $con->where("date_format(assinante.cad, '%Y-%m') = " . $con->escape($mes), NULL, FALSE);
        }
        $con->where("assinante.cad <= assinante.{$instal}", NULL, FALSE);
        $con->where("assinante.cad IS NOT NULL", NULL, FALSE);
        $con->where("assinante.{$instal} IS NOT NULL", NULL, FALSE);
    }
    ...
    function cadInstDiffFaixas($mes) {
        ...
        $areas = $con->select('area.id,area.name,area.name_abbr abbr,area.color')->order_by('name')->get('area')->result_array();
        ...
        foreach ($faixas as $faixaIndex => $faixa) {
            ...
            $series = array(
                'name' =>
                (($from && $to) ? "De {$from} à {$to} dias" : ((!$to) ? "A partir de {$from} dias" : "Menos de {$to} dias")),
                'data' => array(),
                'color' => $faixa['color']
            );
            ...
            foreach ($areas as $j => $area) {
                ...
                if ($area['id']) {
                    $con->where('area.id', $area['id']);
                }
                if ($from) {
                    $con->where("{$diffStr} >=", $from);
                }
                if ($to) {
                    $con->where("{$diffStr} <", $to);
                }
                $con->join('per', 'per.id = assinante.per');
                $con->join('area', 'per.area = area.id');
                $count = $con->count_all_results('assinante');
    ...
    function evCount($d, $areaID = null) {
        ...
        if ($areaID) {
            ...
            $evCount = $con->query($magic)->row_array();
        } else {
            $evCount = $con->query(
                            "SELECT count(*) c
			from tt
    ...
    function ranking_de_causas_por_area($area = 'SIM', $month = 'Total') {
        $con->select('CAUSA, count(*) as c')->from('pita')->where('STATUS_OS !=', 'CANCELADO')->where('CAUSA !=', '');
        if ($area != 'SIM')
            $con->join('per', 'per.id = pita.PER')->join('area', 'per.area = area.id')->where('area.name', $area);
        if ($month != 'Total')
            $con->like('pita.DT_INGR', $month);

        return $con->group_by('CAUSA')->order_by('c', 'DESC')->get()->result_array();
    }
    ...
    function cep_logradouro($cep, $cidade = false) {
        $con->select('logradouro,tp_logradouro');
        $con->where('cep', $cep);
        if ($cidade)
            $con->where('cidade', $cidade);
        $cp = $con->get('cep')->row_array();
        if ($cp)
    ...
    function ranking_de_cep_por_node($node, $month = 'Total') {
        ...
        $magic = "SELECT end as logradouro, bairro, cep, per.name as cidade, count(*) as c
			FROM pita
			JOIN per ON per.id = pita.PER
			LEFT JOIN assinante ON (assinante.cod = pita.NUM_ASS AND assinante.per =  pita.PER)
			WHERE
				pita.NODE =  " . $con->escape($node) .
                (($month != 'Total') ? " AND  pita.DT_INGR  LIKE '" . $con->escape_like_str($month) . "%'" : ""
                ) .
                "GROUP BY assinante.cep
			ORDER BY c desc";
        $ceps = $con->query($magic)->result_array();
    ...
    function node_info($node) {
        $magic = "select area.name as area
		from node
		JOIN per ON node.per = per.id
		JOIN area ON per.area = area.id
		WHERE
			node.node = '$node'";
        return $con->query($magic)->row_array();
    }

    function arquiva_notificacao_de_acompanhamento($acomp) {
        $notif = $con->select('acomp_notif.acomp as acomp,acomp_notif.user as user,acomp_notif.date as date')->join('acomp_update', 'acomp_notif.acomp = acomp_update.id')->where('acomp_update.parent', $acomp)->get('acomp_notif');
        if ($notif)
            $notif = $notif->row_array();
        foreach ($notif as $n)
            $con->insert('notif_log', $n);
        if ($notif)
            return $con->query("DELETE acomp_notif.* FROM acomp_notif
    ...
    function arquiva_notificacao_de_assinante($ass, $per) {
        $notif = $con->get_where('ass_notif', array('ass' => $ass, 'per' => $per))->result_array();
        foreach ($notif as $n)
            $con->insert('notif_log', $n);
        if ($notif)
            return $con->where('ass', $ass)->where('per', $per)->delete('ass_notif');
        else
            return true;
    }
    ...
    function arquiva_notificacao_de_assinantes_que_sairam_do_critico() {
        ...
        date_sub($date, date_interval_create_from_date_string("{$this->intervalo_corrente} days"));
        $d1 = $date->format('Y-m-d');
        $ns = $con->select('ass,per,per.area as area')->join('per', 'per.id = ass_notif.per')->group_by(array('ass', 'per'))->get('ass_notif')->result_array();
        foreach ($ns as $n) {
            $magic = "SELECT
				 COUNT(*) as c
			 FROM (`pita`)
    ...
    function novo_acompanhamento($acomp) {
        if ($con->get_where('acomp', array('ass' => $acomp['ass'],
            ...
            if ($con->insert('acomp', $a)) {
                $update = $acomp['update'];
                $update['parent'] = $con->insert_id();
                $update['date'] = date('Y-m-d H:i:s');
                $update['author'] = $this->user['login'];
                $insert_check = $con->insert('acomp_update', $update);
                $update['id'] = $con->insert_id();
                if ($update['stage'] === 2) {
    ...
    function update_acomp_ri($acomp) {
        ...
        $update['date'] = date('Y-m-d H:i:s');
        $update['author'] = $this->user['login'];
        $u = $con->insert('acomp_update', $update);
        $update['id'] = $con->insert_id();
        if ($u) {
            $this->arquiva_notificacao_de_acompanhamento($acomp['parent']);
            $a = $con->where('id', $acomp['parent'])->update('acomp', array(
                'solved_in' => null,
                'status' => $acomp['status'],
                'stage' => $acomp['stage']
            ));
    ...
    function cria_acks($acomp) {
        $ass = $con->select('ass,per')->where('acomp.id', $acomp)->get('acomp')->row_array();
        if ($ass) {
            $rs = $con->select('
				NRO_OS as os,
				PER as per,
    ...
    function contagem_de_vts_por_assinante($ass, $per) {
        $con->select('COUNT(*) as c');
        $con->where('ass', $ass);
        $con->where('per', $per);
        $con->group_by(array('ass', 'per'));
        $a = $con->get('repair')->row_array();
        return $a['c'];
    }
        ...
        if ($area != 'SIM') {
            ...
            foreach ($this->areas['id'] as $a) {
				...
				WHERE
					c > " . ($a['max_vts'] - 1) . "
				LIMIT 3";
                $y = $con->query($magic)->result_array();
                if ($y)
                    $x = array_merge($x, $y);
            }
            //dumb sort
    ...
    function assinante_nome_e_node($ass, $per) {
        return $con->select('NOME_ASSINANTE as nome,NODE node')->get_where('pita', array('NUM_ASS' => $ass, 'PER' => $per))->row_array();
    }
    ...
    function lista_acomp_aberta_para_assinante($ass, $per) {
        return $con->select('acomp.id,acomp.author,acomp.date,acomp_status.name as status')->join('acomp_status', 'acomp_status.id = acomp.status')->where('acomp.ass', $ass)->where('acomp.per', $per)->get('acomp')->result_array();
    }
    ...
    function ass_hist($ass, $per) {
        $con->select(
				...
				OBS_TECNICO");
        $con->from('pita');
        $con->where('NUM_ASS', $ass);
        $con->where('PER', $per);
        $con->order_by('DT_INGR', 'DESC');
        $r = $con->get();
        return $r->result_array();
    }
    ...
    function lista_reclamacoes_por_cep($cep, $node, $month = 'Total') {
        $magic = "select
			...
			JOIN area ON area.id = per.area
			where
					assinante.cep = " . $con->escape($cep) . "
					and pita.NODE = " . $con->escape($node);
        if ($month != 'Total')
            $magic .= " AND  pita.DT_INGR LIKE '" . $con->escape_like_str($month) . "%'";
        $magic .= "
				ORDER BY DT_INGR
				LIMIT 5000";
        $r = $con->query($magic);
    ...
    function acomps($area = false, $status = false, $stage = false, $month = false) {
		...
		WHERE
			1 = 1
			" . (($area) ? "AND area.id = " . $con->escape($area) : '') . "
			" . (($status !== false && $status !== '') ? "AND `acomp_status`.`id` = " . $con->escape($status) : '') . "
			" . (($stage) ? "AND `acomp_stage`.`id` = " . $con->escape($stage) : '') . "
			" . (($month) ? "AND date_format(acomp.date,'%Y-%m') = " . $con->escape($month) : '') . "
    ...
    function revisita_batch($log_file) {
        ...
        $con->empty_table('revisita');
        $d = (60 * 60 * 24);
        $a = $con->select('NUM_ASS,PER')->group_by(array('NUM_ASS', 'PER'))->get('pita')->result_array();
        ...
        foreach ($a as $ass) {
            ...
            foreach ($oss as $i => $x) {
                ...
                write_file($path, $con->insert_string('revisita', $rev) . ";\n", 'a');
            }
        }
        import_to_met($path);
        unlink($path);
        ...
        $magic = "SELECT
				...
				(
					...
					WHERE
						...
						" . (($mes) ? 'AND date_format(DT_INGR,"%Y-%m") = ' . $con->escape($mes) : '') . "
						AND per.area = {$t['area']}
				) as ds,
				area,
				color,
			...
			FROM (
			SELECT
				" . (($rev) ? "IF(ORDINAL>0 OR NOT (STATUS_OS = 'CANCELADA' OR STATUS_OS = 'FINALIZADA'),0,
						IF(prevDIFF IS NULL,0,
							IF(prevDIFF <= " . $this->intervalo_revisita . ",1,0)
						)
					) rev" : "IF(ORDINAL>0 OR NOT (STATUS_OS = 'CANCELADA' OR STATUS_OS = 'FINALIZADA'),0,
			...
			WHERE
				TEC " . (($t['TEC'] !== NULL) ? "= '{$t['TEC']}'" : 'IS NULL') . "
				AND per.area = {$t['area']}
				" . (($mes) ? "AND date_format(pitaDT,'%Y-%m') = " . $con->escape($mes) : "") . "
			) x
    ...
    function _prod_x_quali($mes, $area, $rev) {
        ...
        if ($mes)
            $con->where("date_format(DT_INGR,'%Y-%m') = " . $con->escape($mes), NULL, FALSE);
        if ($area)
            $con->where('per.area', $area);

        $tecs = $con->get('pita')->result_array();
    ...
    function grpper($per) {
        $x = $con->select('grp')->where('id', $per)->get('per')->row_array();
        if ($x) {
            return intval($x['grp']);
        } else {
            return $per;
    ...
    function os_cache_merge($log_file) {
        ...
        $c = 0;
        $zero = time();
        if ($con->count_all_results('os_cache') === 0) {
        ...
        if ($o['svc'] === 'cm')
            $magic = "SELECT
				...
				FROM (
					...
					" . ((array_key_exists('ini', $o)) ? "AND GXVSIM.IORDENES.IORDFCHING > '{$o['ini']}'" : ''
                    ) . "
				) x
    ...
    function os_cache_format($y, $o) {
        ...
        $result = array(
            ...
            'svc' => $o['svc'],
            'tecnico' =>
            ( ( array_key_exists('TEC', $y) && $y['TEC'] ) ? fCap(trim($y['TEC'])) : null
            ),
            ...
            'ag' =>
            (
            (!$y['AG'] || !trim($y['AG']) || trim($y['AG']) === '01-JAN-01') ? null : date('Y-m-d', strtotime(trim($y['AG'])))
            ),
            'asscod' => $y['ASSCOD'],
            'assname' => fCap($y['ASS']),
            ...
            'obs_tec' => $y['OBS_TEC'],
            'falha' => fCap($y['FALHA']),
            'causa' => ((array_key_exists('CAUSA', $y) && $y['CAUSA']) ? fCap($y['CAUSA']) : NULL),
    ...
    function _sigaConn() {
        ...
        $con->insert('siga_conn', array(
            'minute' => intval(($t - strtotime($day)) / 60),
            'day' => $day,
            'user' =>
            (($this->user && $this->user['login']) ? $this->user['login'] : null
            ),
        ...
        $h = intval(date('G'));
        if ($h < 7 && $h > 0) {
            $con->where('DATEDIFF(current_date,day) >', 7)->delete('siga_conn');
        }
    }
    ...
    function calc_new_base($log_file) {
        $mm = date('Y-m') . '-01';
        $con->query("DELETE FROM cbase WHERE month = '{$mm}'");
        $magic = "INSERT INTO cbase
        ...
        return $con->query($magic);
    }

    function per_group($per) {
        $grp = $con->select('grp')->where('id', $per)->get('per')->row_array();
    ...
    function os_has_schedule($os) {
        return ($con->where('os', $os['os'])->where('per', $os['per'])->where('svc', $os['svc'])->count_all_results('tec_schedule_os') > 0);
    }

    function reset_dates($log_file) {
        $con->query('TRUNCATE TABLE dates');
        $date = new DateTime($this->begin);
        $d = $date->format('Y-m-d');
        while ($d <= date('Y-m-d')) {
            $con->insert('dates', array('d' => $d));
            date_add($date, date_interval_create_from_date_string("1 days"));
            $d = $date->format('Y-m-d');
        }
        // ~~~ ~~ ~~
        $con->query('TRUNCATE TABLE mes');
        $date = new DateTime($this->begin);
        while ($date->format('Y-m') <= date('Y-m')) {
            $con->insert('mes', array(
                'm' => $date->format('Y-m'),
    ...
    function lday_backlog() {
        $lday = $con->select('MAX(DATE(DT_GERACAO)) as d', FALSE)->get('backlog')->row_array();
        if ($lday && $lday['d'])
            return $lday['d'];
        else
            return date('Y-m-d');
    ...
    function lday_backlog_r() {
        $d = $con->select_max('d')->get('backlog_log')->row_array();
        if ($d)
            return $d['d'];
        else
            return date('Y-m-d');
    ...
    function backlog_r_h() {
        $set = $con->query('SELECT MIN(d) as d1, MAX(d) as d2 FROm backlog_log')->row_array();
    ...
    function backlog_extract($a, $d, $s) {
        ...
        if ($a)
            $con->where('per.area', $a);
        if ($d)
            $con->like('DT_GERACAO', $d);
        if ($s)
            $con->where('STATUS', $s);
        $r = $con->get('backlog')->result_array();
        if ($r) {
            $this->load_supsiga();
            foreach ($r as $i => $x) {
    ...
    function backlog_aging_r($d, $a) {
        ...
        $b = $con->query($magic)->result_array();
    ...
    function os_open($o, $st = 'P', $dtf = 'd/m/y', $show_future = false) {
        ...
        if (strtolower($o['svc']) === 'cm') {
            ...
            $con->where('per', $o['per']);
            $con->where('svc', 'cm');
            $con->update('os_cache', $oscache);

            $con->where('NRO_OS', $o['os']);
            $con->where('PER', $o['per']);
            $con->where('SERVICO', 'CM');
            $con->update('pita', array(
        ...
        if (!array_key_exists($r['per'], $pers)) {
            $pers[$r['per']] = $con->where('id', $r['per'])->get('per')->row_array();
        }
    ...
    function geo_search($cep, $end) {
        return $con->where('cep', str_replace('-', '', $cep))->where('end', strtolower($end))->get('geo_cache')->row_array();
    }
    ...
    function os_get(
    $o, $fulladdress = false, $geo = false, $turno = false, $ignorecache = false, $inhuman = false
    ) {
        $this->load_supsiga();
        ...
        if ($r) {
            ...
            $r = array(
                ...
                'ag' => (($r['AG'] && trim($r['AG']) > '01-JAN-01') ? date((($inhuman) ? 'Y-m-d' : 'd/m/Y'), strtotime(trim($r['AG']))) : null
                ),
                'cumprimento' =>
                (($r['CUMPRIMENTO'] && trim($r['CUMPRIMENTO']) > '01-JAN-01') ? date(
                                (($inhuman) ? 'Y-m-d' : 'd/m/Y'), strtotime(trim($r['CUMPRIMENTO']))
                        ) . ' ' . h_parse($r['HORA_INI']) . ' ~ ' . h_parse($r['HORA_FIM']) : null
                ),
                'falha' => fCap($r['FALHA']),
                'causa' => ((array_key_exists('CAUSA', $r) && $r['CAUSA']) ? fCap($r['CAUSA']) : NULL),
                'tel' => fTel($r['TEL'], $r['TELCOD']),
                'cel' => fTel($r['CEL'], $r['CELCOD']),
                ...
                'pacote_tipo' =>
                (array_key_exists('PACOTE_TIPO', $r) ? pacTipo($r['PACOTE_TIPO']) : null),
            ...
            if ($fulladdress) {
                $per = $con->where('id', $r['per'])->get('per')->row_array();
                $r['cid'] = $per['name'];
                $r['uf'] = $per['uf'];
            }
            if ($geo) {
    ...
    function os_tt_attached($o) {
        ...
        if ($tt1) {
            $tt1['loc'] = $con->where('ok', 0)->where('tt', $tt1['id'])->get('tt_location')->result_array();
        ...
        if ($tt2) {
            foreach ($tt2 as $tt) {
                $tt['loc'] = $con->where('ok', 0)->where('tt', $tt['id'])->get('tt_location')->result_array();
        ...
        $list = $con->query(
			...
			WHERE
				...
				AND os_cache.tipo = 'reclamação'"
					...
					AND tt_os_ack.id IS NULL" : "AND os_cache.status != 'cancelada'") . "
    ...
    function from_ri($os) {
        return $con->where('os', $os['os'])->where('per', $os['per'])->where('svc', $os['svc'])->from('tt_os_ack_trash')->count_all_results();
    }
    ...
    function pita_range($id, $level = 'node', $filter = true, $range = false) {
        ...
        $list = $con->query(
			...
			WHERE
				" . (($range) ?
                                "DATE(ingr) BETWEEN '" . date('Y-m-d', strtotime($range['from'])) . "' AND '" . date('Y-m-d', strtotime($range['to'])) . "'" : "ingr >= '{$d}'") . "
    ...
    function _node_avg($node, $weekDays) {
        ...
        foreach ($weekDays as $d) {
            $query = "SELECT avg(c) m
				FROM (
					SELECT COUNT(*) c
					FROM pita
					WHERE
						node = " . $con->escape($node) . "
						AND dt_ingr > '{$minDt}'
						AND DAYOFWEEK(dt_ingr) = {$d}
						AND status_os != 'CANCELADA'
					GROUP BY dt_ingr
				) x";
            $avg = $con->query($query)->row_array();
            if ($avg) {
                $val += floatval($avg['m']);
            }
        }
    ...
    function mon_list() {
		...
		GROUP BY os_cache.svc,os_cache.per,os_cache.os
		ORDER BY ingr ASC";
        $list = $con->query($magic)->result_array();
        foreach ($list as $x) {
            ...
            if (!array_key_exists($node, $result['areas'][$per]['nodes'])) {
                $result['areas'][$per]['nodes'][$node] = array(
                            'id' => $node,
                            'name' => $node,
                            'count' => 0,
                            'dashboard' => uri_hash('dashboard', array(
            ...
            if (!array_key_exists($cep, $result['areas'][$per]['nodes'][$node]['ceps'])) {
                ...
                $result['areas'][$per]['nodes'][$node]['ceps'][$cep] = array(
                            'id' => $cep,
                            'name' => $logr,
                            'descr' => $bairro,
                            'count' => 0,
    ...
    function nodes_crit($area = false) {
        ...
        foreach ($aux as $i => $x) {
            ...
            $x['agz'] = ($x['ag_min'] && $x['ag'] >= $x['ag_min'] && !$x['pez']);
    ...
    function backlog_pendente($corp, $per = false) {
        $fff = function($os) {
            return array(
                'per' => intval($os['per']),
                'os' => intval($os['os']),
                'ag' => (($os['ag'] && $os['ag'] >= '2012-01-01') ? strtotime($os['ag']) : 0
                ),
        ...
        $yy = $con->query($magic)->result_array();

        if ($yy) {
            $yy = array_map($fff, $yy);
            $xx = array_merge($xx, array_filter($yy, function($os) use ($xx) {
                        $unique = true;
                        foreach ($xx as $k) {
                            if (
                                    $k['os'] === $os['os'] && $k['per'] === $os['per'] && $k['svc'] === $os['svc']
                            ) {
                                $unique = false;
                                break;
                            }
    ...
    function backlog_status($st, $corp, $per = false, $show_future = false) {
        ...
        //from cache
        $con->select('per,os,svc,ag,ingr');
        if ($per)
            $con->where('per', $per);

        if (!$show_future) {
            if ($st === 'A') {
                $con->where("ag <=", date('Y-m-d'));
            }
            if ($st === 'E') {
                $con->where("(ag < '" . date('Y-m-d') . "' OR ag IS NULL)", NULL, FALSE);
            }
        }


        $con->where("asstipo" . (($corp) ? " !=" : ''), 'individual'); //tipo de assinante
        $con->where('status', strtolower(stName($st))); //status os
        $con->where('tipo', 'reclamação');
        $con->order_by('per,os');
        $xx = $con->get('os_cache')->result_array();
        ...
        //from pita
        $con->select("PER as per,NRO_OS as os,SERVICO as svc, DT_AGEND as ag, DT_INGR as ingr");
        $con->where('DT_INGR <=', $d->format('Y-m-d'));
        if ($per) {
            $con->where('PER', $per);
        }
        if (!$show_future) {
            if ($st === 'A') {
                $con->where("DT_AGEND <=", date('Y-m-d'));
            }
            if ($st === 'E') {
                $con->where("( DT_AGEND < '" . date('Y-m-d') . "' OR DT_AGEND IS NULL )", NULL, FALSE);
            }
        }

        $con->where("TIPO_ASS" . ((!$corp) ? " !=" : ''), 'mestre'); //tipo de assinante
        $con->where('STATUS_OS', strtolower(stName($st))); //status os
        $con->order_by('per,os');
        $yy = $con->get('pita')->result_array();
    ...
    function loc_tree() {
        $nodes = $con->select('node.node,per.abbr as per,per.name as per_name')->join('per', 'per.id = node.per')->order_by('per_name,node')->get('node')->result_array();
        $lper = -1;
        $i = -1;
        $t = array();
        foreach ($nodes as $n) {
    ...
    function perms_needed($page) {
        $p = $con->select('perms')->get_where('page_perms', array('page' => $page))->row_array();
        if ($p)
            return $p['perms'];
        else
            return null;
    ...
    function lastttu($tt) {
        return $con->order_by('id', 'desc')->limit(1)->get_where('tt_update', array('tt' => $tt))->row_array();
    }
    ...
    function tt_group_users($g, $per = null) {
        $con->
                ...
                where('group', $g);
        if ($per) {
            $con->where(" ( tt_user.per IS NULL OR tt_user.per LIKE '%{$per}%' ) ", NULL, FALSE);
        }
        return $con->get('tt_user')->result_array();
    }

    function node_eventos_ranking($mes = false, $per = false) {
        if ($mes)
            $mes = date('Y-m', strtotime($mes));
        $magic = "select tt_location.location node,count(*) a
			from tt_location
			join tt ON tt_location.tt = tt.id
			JOIN node ON node.node = tt_location.location
			JOIN per ON per.id = node.per
			where
				tt.type != 'backlog'
				AND tt.status != 'cancelado' " .
                (($mes) ? "AND date_format(tt.open,'%Y-%m') = " . $con->escape($mes) : ""
                ) .
                (($per) ? " AND per.abbr = " . $con->escape($per) : ''
                ) .
                " AND tt_location.location_type = 'node'
			group by tt_location.location
			order by a desc,node asc";
        $nodes = $con->query($magic)->result_array();
        return $nodes;
    }
    ...
    function node_mon_ranking($mes = false, $per = false) {
        ...
        if ($mes)
            $con->where('date_format(ini,"%Y-%m") = ' . $con->escape($mes), NULL, FALSE);
        if ($per)
            $con->where('per.abbr', $per);
        return $con->group_by('node')->order_by('c', 'desc')->order_by('node', 'asc')->get('crit_node')->result_array();
    }
    ...
    function cidade_evento_ranking($mes = false) {
        ...
        $pers = $con->query($sql)->result_array();
        return $pers;
    }
    ...
    function cidade_mon_ranking($mes = false) {
        ...
        $pers = $con->query(
			...
			LEFT JOIN (
				...
				from crit_node
					" . (($mes) ? "where
							date_format(ini,'%Y-%m') = " . $con->escape($mes) : ""
                        ) . "
				group by per
    ...
    function evento_x_dia($per) {
        if ($per) {
            $magic = "SELECT d, COUNT(DISTINCT id) c
				FROM (
					SELECT
						tt.id,
						DATE(tt.open) d
        ...
        $ds = $con->query($magic)->result_array();
        return $ds;
    }
    ...
    function vend_inst_fi($area = false, $d = false) {
        ...
        if ($d === 'Total')
            ...
            $d = date('Y-m', strtotime($d));
        $magic = "select FORMA_INGRESSO f, COUNT(*) c
			from vend_log
			" . (($area) ? "join per ON per.id = COD_OPERADORA" : "") . "
			WHERE
				VENDA IS NULL
    ...
    function vend_inst_ta($area = false, $d = false) {
        ...
        if ($d === 'Total')
            ...
            $d = date('Y-m', strtotime($d));
        $magic = "select TIPO_ASS t, COUNT(*) c
			from vend_log
			" . (($area) ? "join per ON per.id = COD_OPERADORA" : "") . "
			WHERE
				VENDA IS NULL
    ...
    function _per_tecs($per) {
        ...
        if (!$tecs)
            $tecs = array();
        $tt = array();
        $tecs = array_map(function($t) use (&$tt, $per) {
            $tn = fLower($t['NAME']);
            $tt[] = $tn;
            return array(
                'name' => $tn,
        ...
        $itecs = array_map(function($t) use($per) {
            $tn = fLower($t['NAME']);
            return array(
                'name' => $tn,
                'per' => $per
        ...
        $tecs = array_merge($tecs, array_filter($itecs, function($t) use($tt) {
    ...
    function get_tec($name, $per, $create = true) {
        $t = $con->where('name', $name)->where('per', $per)->get('tec_aloc')->row_array();
        if ($t) {
            return $t;
        } elseif ($create) {
            $con->insert('tec_aloc', array('name' => $name, 'per' => $per));
            return $con->where('id', $con->insert_id())->get('tec_aloc')->row_array();
        } else {
            return null;
        }
    }
    ...
    function geo_tecs($tecs, $d) {
        foreach ($tecs as $i => $t) {
            $x = $con->select('AVG(lat) lat,AVG(lng) lng')->where('day', $d)->where('lat IS NOT NULL', null, false)->where('lng IS NOT NULL', null, false)->where('tec', $t['id'])->get('tec_schedule')->row_array();
            if ($x && $x['lat'] && $x['lng']) {
                ...
                if ($t['zona']) {
                    $z = $con->select('lat,lng')->where('id', $t['zona'])->get('zona')->row_array();
                    if ($z) {
    ...
    function os_tipo_tecs($tecs) {
        foreach ($tecs as $i => $t) {
            $x = $con->select('os_tipo as tipo')->where('tec', $t['id'])->get('os_tipo_tec')->result_array();
            $tecs[$i]['tipos'] = array();
            if ($x)
    ...
    function tecs_battery_level($tecs, $d) {
        ...
        foreach ($tecs as $i => $t) {
            ...
            if (date('Y-m-d') === $d) {
                $con->where('time >', date('Y-m-d H:i:s', time() - 60 * 60));
            }
    ...
    function tecs_workgrid_bounds($tecs, $d) {
        ...
        foreach ($tecs as $i => $t) {
            $x = $con->select("MIN(ini) as turno_ini,MAX(end) as turno_end")->
                            where('weekday', $w)->
                            where('tec', $t['id'])->
                            where('working', 1)->
    ...
    function tecs_workgrid($tecs, $d) {
        $w = date('N', strtotime($d));
        foreach ($tecs as $i => $t) {
            $x = $con->select("ini,end,weekday,tec,working")->
                            where('weekday', $w)->
                            where('tec', $t['id'])->
    ...
    function per_timezone($per) {
        $x = $con->select('timezone')->where('id', $per)->get('per')->row_array();
        return $x['timezone'];
    }
        ...
        foreach ($tecs as $i => $t) {
            ...
            if ($x) {
                foreach ($x as $j => $y) {
                    ...
                    if ($y['activity'] === 'os') {
                        $m = $con->select('id,os,per,svc,os_tipo,turno')->where('tec_schedule', $y['id'])->get('tec_schedule_os')->result_array();
                        if ($m)
                            $x[$j]['pack'] = $m;
                        else
                            $x[$j]['pack'] = array();
    ...
    function tecs_lunch($tecs, $d) {
        ...
        foreach ($tecs as $i => $tec) {
            ...
            if ($con->where('tec', $tec['id'])->where('day', $d)->count_all_results('tec_schedule') === 0) {
                if (!array_key_exists($tec['workgrid'], $workgrids))
                    $workgrids[$tec['workgrid']] = $con->where('id', $tec['workgrid'])->get('workgrid')->row_array();
                $con->insert('tec_schedule', array(
                    'tec' => $tec['id'],
                    'activity' => 'lunch',
                    'per' => $tec['per'],
                    'day' => $d,
    ...
    function activities_times($per) {
        $x = $con->where('per', $per)->get('os_tipo_time')->result_array();
        $times = array();
        foreach ($x as $y) {
    ...
    function os_tipoS($abbr = false) {
        $tps = $con->select('name, LEFT(name,1) as abbr', FALSE)->order_by('name')->get('os_tipo')->result_array();
        $r = array();
        foreach ($tps as $t)
            $r[$t['abbr']] = $t['name'];
        return $r;
    ...
    function stperminute() {
        $read = false;
        $x = array();
        $y = $con->where('name', 'os_cache')->get('system_args')->row_array();
        if ($y)
            $read = $y['value'];
        if ($read) {
    ...
    function sys_arg_int($name, $default = 5) {
        $x = $con->select('value')->where('name', $name)->get('system_args')->row_array();
        if ($x)
    ...
    function turno($id) {
        $x = $con->where('id', $id)->get('turno')->row_array();
        return (($x) ? $x : null);
    }
    ...
    function local_agenda($per, $d) {
        ...
        $x = $con->query($magic)->result_array();
        if ($x)
            foreach ($x as $i => $os) {
    ...
    function local_os_get($o, $scheduleID = false) {
        $magic = $this->_local_top_query() .
                "WHERE
				os_cache.os = " . $con->escape($o['os']) . "
				" . (($scheduleID) ? ' AND tec_schedule_os.tec_schedule = ' . $con->escape($scheduleID) : ''
                ) . "
				AND os_cache.per = " . $con->escape($o['per']) . "
				AND os_cache.svc = " . $con->escape($o['svc']) . "";

        $os = $con->query($magic)->row_array();
        if ($os)
    ...
    function agenda($per, $t) {
        ...
        foreach ($x as $os) {
            $new = array(
                        'per' => $per,
                        'os' => intval($os['OS']),
                        'ag' => (($os['AG']) ? strtotime($os['AG'] . ' 00:00:00') : 0
                        ),
    ...
    function siga_motivos() {
        $this->load_supsiga();
        $m = array();
        $m['tv'] = $this->supsiga->select('GXVSIM.REPMOT.REPMOTDSC as "name", GXVSIM.REPMOT.REPMOTCOD as "cod"')->order_by('name')->get('GXVSIM.REPMOT')->result_array();
        $m['cm'] = $this->supsiga->select('GXVSIM.IORDMOT.IORDMOTDSC as "name", GXVSIM.IORDMOT.IORDMOTCOD as "cod"')->order_by('name')->get('GXVSIM.IORDMOT')->result_array();
        return $m;
    }
    ...
    function siga_causas() {
        $this->load_supsiga();
        $c = array();
        $c['tv'] = $this->supsiga->select('GXVSIM.CAUSFALLA.CAUSFALLDSC as "name", GXVSIM.CAUSFALLA.CAUSFALLNRO as "cod"')->order_by('name')->get('GXVSIM.CAUSFALLA')->result_array();
        $c['cm'] = $this->supsiga->select('GXVSIM.ICAUFAL.ICAUSFALLDSC as "name", GXVSIM.ICAUFAL.ICAUSFALLNRO as "cod"')->order_by('name')->get('GXVSIM.ICAUFAL')->result_array();
        return $c;
    }
    ...
    function eventos_abertos($type = false, $perlist = null, $fullPerName = false, $dia = null) {
		...
		WHERE
			...
			AND tt.type != 'backlog'
			" . (($dia) ? " AND DATE(tt.ini) = " . $con->escape($dia) . " " : " AND tt_status.name != 'fechado' 
				AND tt_status.name != 'ativo' "
                ) .
                (($type) ?
                        " AND tt.type = " . $con->escape($type) :
                        ''
                ) . "
		ORDER BY t_order,id";

        $d = $con->query($magic)->result_array();
        if (!$d)
            $d = array();
        foreach ($d as $y => $t) {
            ...
            foreach ($t['pers'] as $per) {
                $p = $con->where('id', $per)->get('per')->row_array();
                if ($fullPerName)
                    $pers[] = $p['name'];
                else
                    $pers[] = $p['abbr'];
    ...
    function _reclamacoes_iar($mesINI, $mesEND, $areaID, $campo = 'ingresso') {
        ...
        if ($areaID)
            $con->join('per', 'per.id = os_fin.per')->where('per.cst_area', $areaID);
        if ($mesINI !== $mesEND) {
            $con->where("os_fin.{$campo} BETWEEN '{$mesINI}' AND '{$mesEND}'", NULL, FALSE);
        } else {
            $con->where("date(os_fin.{$campo}) = " . $con->escape($mesINI), NULL, FALSE);
        }
        return
                        $con->
                        select(
    ...
    $areaID = null, $id, $tipo = null, $dIni = false, $dEnd = false, $cumpr = false
    ) {
        ...
        switch ($id) {
            case 'conforme':
                $selector = "( ( os_fin.offset IS NULL AND os_fin.inside IS NOT NULL )
							OR ( os_fin.offset > -300 AND os_fin.offset < 300 ) )";
                break;
            case 'antes':
                $selector = "os_fin.offset < -300";
                break;
            case 'depois':
                $selector = "DATE(os_fin.real_ini) = DATE(os_fin.min_ini)
            ...
            case 'outro_dia':
                ...
                break;
            case 'indefinido':
                $selector = " ( os_fin.offset IS NULL AND os_fin.inside IS NULL ) ";
                break;
        }
        $magic = "SELECT
				os_fin.os,
			...
			" . (($areaID) ? " AND per.cst_area = " . $con->escape($areaID) : ''
                ) . " )
			WHERE 
				$selector "
                . (($dIni && $dEnd) ? (($dIni === $dEnd) ? " AND DATE(os_fin.{$campo}) = '{$dIni}' " : " AND DATE(os_fin.{$campo}) BETWEEN '{$dIni}' AND '{$dEnd}' ") : ''
                ) . (($tipo) ? ( ( is_array($tipo) ) ? " AND os_fin.tipo IN (" . implode(', ', $tipo) . ") " : " AND os_fin.tipo = " . $con->escape($tipo)
                        ) : ''
                );
        return $con->query($magic)->result_array();
    }
    ...
    function _cja_ind(
    $areaID = null, $tipo = null, $clean = false, $dIni = false, $dEnd = false, $cumpr = false
    ) {
        $campo = ($cumpr) ? 'real_ini' : 'ingresso';
        ...
        $magic = "SELECT 
			...
			FROM (
				...
				WHERE
					1 = 1 "
                . (($dIni && $dEnd) ? (($dIni === $dEnd) ? " AND DATE(os_fin.{$campo}) = '{$dIni}' " : " AND os_fin.{$campo} BETWEEN '{$dIni}' AND '{$dEnd}' ") : ''
                ) . (($tipo) ? ( ( is_array($tipo) ) ? " AND os_fin.tipo IN (" . implode(', ', $tipo) . ") " : " AND os_fin.tipo = " . $con->escape($tipo)
                        ) : ''
                ) . (($clean) ? " AND NOT ( os_fin.offset IS NULL AND os_fin.inside IS NULL ) " : ''
                ) . " ) x";
        ...
        if ($os['svc'] === 'cm') {
            ...
            $ag = trim_em_all(
                    $this->supsiga->
                            ...
                            where('RECNRO', $os['os'])->
                            where('PERCOD', $os['per'])->
                            get('REPARA')->row_array(), true);
        }

        if ($ag)
            $turno = $con->where('id', $ag['turno'])->get('turno')->row_array();
