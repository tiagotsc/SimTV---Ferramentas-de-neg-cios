...
class viabilidade extends Base {
    ...
    public function __construct(){
        ...
        $this->logDados['modulo'] = 'Telecom/viabilidade';
    ...
    function viabilidadePedidos()
    { 
        #echo 'aqui'; exit();

        #$this->menuLateral = $this->dadosBanco->menuLateral('TELECOM', $this->session->userdata('permissoes'));
        #$this->util->setPositionMenu('');
        #$this->menuLateral = $this->util->montaMenuLateral($this->dadosBanco->menuLateralDropDown('TELECOM', $this->session->userdata('permissoes')), $this->dadosBanco->paisMenuLateralDropDown('TELECOM', $this->session->userdata('permissoes')));
        $this->layout->region('html_header', 'view_html_header');
    ...
    public function pesqTeste(){
        
        #$this->output->enable_profiler(TRUE);
        $this->crud->setConexao('adminti');
        $this->crud->setTabela('usuario');
        $this->crud->setRelacao('cd_departamento', 'departamento', 'nome_departamento');
        #$crud = $this->crud->setRelacao('cd_perfil', 'perfil', 'nome_perfil');
        $this->crud->setListaCamposSel(array('usuario.nome_usuario' => 'nome', "usuario.data_criacao_usuario" => 'dt','usuario.email_usuario' => 'email', 'departamento.nome_departamento' => 'departamento'));
        $crud = $this->crud->listar();
        $dados['menuLateral'] = $this->menuLateral;
        $this->layout->region('html_header', 'view_html_header');
      	#$this->layout->region('menu', 'view_menu', $menu);
        $this->layout->region('menu_lateral', 'view_menu_lateral', $dados);
        $this->layout->region('corpo', 'viabilidade/view_psq_viabilidade', $crud);
      	$this->layout->region('rodape', 'view_rodape');
      	$this->layout->region('html_footer', 'view_html_footer');
