                ...
                <div class="modal-body">
                    <?php              
                        ...
                        echo '<div class="row">';
                    		...
                    		echo form_dropdown('tipo', $options, '', 'id="tipo_status" class="form-control"');
                ...
                <div class="modal-body">
                    <?php              
                        ...
                        echo '<div class="row">';
                    		...
                    		echo form_label('Tipo', 'tipo');
                    		echo form_dropdown('tipo', $options, '', 'id="tipo_status_todos" class="form-control"');
            ...
            <div class="col-lg-12">
                ...
                <div id="divMain">
                    <?php
                    	...
                    	echo form_open('ura/ura/pesq',$data);
                    		...
                    		echo form_fieldset("Pesquisar node".$botaoCadastrar, $attributes);
                                ...
                                echo '<div class="row">';
                            		...
                            		echo form_label('Permissor', 'percod');
                            		echo form_dropdown('percod', $options, $percod, 'id="percod" class="form-control"');
                                    echo '</div>';
                                    
                                    echo '<div class="col-md-3">';
                                    #print_r(array_keys($listaBancos));
                                    echo form_label('N&uacute;mero', 'nodeDsc');
                        			$data = array('name'=>'nodeDsc', 'value'=>$nodeDsc,'id'=>'nodeDsc', 'placeholder'=>'Digite n&uacute;mero', 'class'=>'form-control');
                        			echo form_input($data);
                                    echo '</div>';
                                ...
                                echo form_submit("btn",utf8_encode("Pesquisar"), 'class="btn btn-primary pull-right"');
                                echo '</div>';
                ...
                foreach($campos as $nome){     
                    ...
                    if($nome == $id){
                        ...
                        $colunas[] = anchor('ura/ura/pesq/'.$post.'/'.$nome.'/'.(($sort_order == 'asc' && $sort_by == $nome) ? 'desc' : 'asc') ,$label.$icoAscDesc, array('class' => $class));
                ...
                foreach($dados as $dNode){

                    $cell[$contCell++] = array('data' => '<input type="checkbox" class="pegaCheckbox" name="node[]" value="'.$dNode->id.'">');
                    $cell[$contCell++] = array('data' => $dNode->id);                    
                    $cell[$contCell++] = array('data' => htmlentities($this->dadosBanco->unidade($dNode->percod)[0]->nome));
                    $cell[$contCell++] = array('data' => $dNode->nodeDsc);
                    $cell[$contCell++] = array('data' => ($dNode->tipo != '')? $dNode->tipo: '' );
                    $cell[$contCell++] = array('data' => $this->util->formataData(substr($dNode->dataInicio, 0, 10),'BR').' '.substr($dNode->dataInicio, 11, 5));
                    #$cell3 = array('data' => $usu->email_usuario);
                    $cell[$contCell++] = array('data' => $this->util->formataData(substr($dNode->dataFim, 0, 10),'BR').' '.substr($dNode->dataFim, 11, 5));
                    $cell[$contCell++] = array('data' => htmlentities($dNode->origem));
                    #$cell6 = array('data' => '<a href="#" class="altStatus" id="'.$dNode->id.'">'.$dNode->status.'</a>');
                    $cell[$contCell++] = array('data' => $dNode->status);
                    ...
                    $botaoEditar = (in_array(262, $this->session->userdata('permissoes')))? '<a title="Mudar status" href="#" onclick="statusRegistro('.$dNode->id.', \''.$dNode->nodeNro.' - '.$dNode->nodeDsc.'\',\''.$this->util->formataData(substr($dNode->dataFim, 0, 10),'BR').'\',\''.substr($dNode->dataFim, 11, 5).'\', \''.$dNode->status.'\',\' '.$dNode->tipo.' \')" data-toggle="modal"  data-target="#alt-status" class="glyphicon glyphicon-pencil"></a>': '';
                    
                    $cell[$contCell++] = array('data' => $botaoEditar);
...
function statusRegistro(cd, nome, datafim, horafim, status, tipo){ 
    $("#alt_cd_node").val(cd);
    $("#alt_nome_node").val(nome);
    $("#alt_data_fim").val(datafim);
    $("#alt_hora_fim").val(horafim);
    $("#status").val(status);
    $("#tipo_status").val($.trim(tipo));
...
$('#alt_hora_fim,#alt_hora_fim_todos').timepicker({
