    ...
    <!-- FIM Modal Apaga registro de telecom -->
            ...
            <!--<div class="col-lg-12">-->
                ...
                <div id="divMain">
                    <?php
                    	...
                    	echo form_open($this->session->userdata('indexPHP').$pasta.'/'.$controller.'/'.$metodo,$data);
                            ...
                            $botaoCadastrar = (in_array($perEditarCadastrar, $this->session->userdata('permissoes')))? "<a href='".base_url($this->session->userdata('indexPHP').$pasta.'/'.$controller.'/ficha'.ucfirst($controller))."' class='linkDireita'>Cadastrar&nbsp<span class='glyphicon glyphicon-plus'></span></a>": '';
                ...
                foreach($dados as $da){
                    ...
                    $botaoImprimir = (in_array($perImprimir, $this->session->userdata('permissoes')))? '<a title="Imprimir" target="_blank" href="'.base_url($this->session->userdata('indexPHP').$pasta.'/'.$controller.'/imprimirOrdem/'.$this->util->base64url_encode($da->$pk)).'" class="glyphicon glyphicon-print"></a>': '';
...
$(".reenvio").click(function(){
    ...
    if (r == true) {
        $(this).carregando();
        url = "<?php echo base_url($this->session->userdata('indexPHP').$pasta.'/'.$controller.'/reEnviarOrdemEmail');?>/"+$(this).attr('ordem');
