    ...
    <!-- FIM Modal Apaga registro de telecom -->
            ...
            <!--<div class="col-lg-12">-->
                ...
                foreach($dados as $da){
                    ...
                    $botaoImprimir = (in_array($perImprimir, $this->session->userdata('permissoes')))? '<a title="Imprimir" target="_blank" href="'.base_url($pasta.'/'.$controller.'/pdf/'.$da->$pk).'" class="glyphicon glyphicon-print"></a>': '';
                    $botaoEditar = false;#(in_array($perEditarCadastrar, $this->session->userdata('permissoes')))? '<a title="Editar" href="'.base_url($pasta.'/'.$controller.'/ficha/'.$da->$pk).'" class="glyphicon glyphicon-pencil"></a>': '';
                    $botaoExcluir = false;#(in_array($perExcluir, $this->session->userdata('permissoes')))? '<a title="Apagar" href="#" data-toggle="modal" data-target="#apaga" key="'.$da->$pk.'" nome="'.$nome.'" class="del glyphicon glyphicon-remove"></a>': '';
