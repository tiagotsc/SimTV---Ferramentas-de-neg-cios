 ...
 * Dashboard_model
 * 
 * Classe que realiza o tratamento do m�dulo de node
 * 
 * @package   
...
class Node_model extends CI_Model{
    ...
    * Node_model::insere()
    * 
    * Fun��o que realiza a inser��o dos dados da operadora na base de dados
    ...
    */
	public function insere(){
        ...
        if($existe){
            ...
            redirect(base_url('tcom-node/node/ficha'));
            exit();
        }
		...
		$sql = "INSERT INTO ".BANCO_TELECOM.".tcom_node (".$campos.")\n VALUES(".$valores.");";
		$this->db->query($sql);
    ...
    * Node_model::atualiza()
    * 
    * Fun��o que realiza a atualiza��o dos dados da operadora na base de dados
    * @return O n�mero de linhas afetadas pela opera��o
    */
	public function atualiza(){
		...
		$sql = "UPDATE ".BANCO_TELECOM.".tcom_node SET ".$camposValores." WHERE id = ".$this->input->post('id').";";
		$this->db->query($sql);
    ...
    public function verificaNode($node, $descricao){
        ...
        $this->db->where('node', $node);
        $this->db->where('descricao', $descricao);
        return $this->db->get(BANCO_TELECOM.'.tcom_node')->num_rows();
        
    }
    
    public function gravaCelula($id){
        
        $sql = "Delete FROM ".BANCO_TELECOM.".tcom_nodeCelula WHERE idNode = ".$id;
        $this->db->query($sql);
        ...
        if($this->input->post('celula')){
            ...
            foreach($this->input->post('celula') as $celula){
                ...
                $sql = "INSERT INTO ".BANCO_TELECOM.".tcom_nodeCelula (idNode, tipo) VALUES(".$id.", '".$celula."')";
                $this->db->query($sql);
    ...
    * Node_model::dados()
    * 
    * Fun��o que monta um array com todos os dados da operadora
    * @param $cd Cd da operadora para recupera��o de dados
    * @return Retorna todos os dados da operadora
    */
	public function celulasNode($id){
        
            $this->db->where('idNode', $id);
            $dados = $this->db->get(BANCO_TELECOM.'.tcom_nodeCelula')->result_array(); # TRANSFORMA O RESULTADO EM ARRAY
     ...
     * Node_model::pesquisa()
     * 
     * lista as operadoras existentes de acordo com os par�metros informados
    ...
    public function pesquisa($parametros, $mostra_por_pagina, $sort_by, $sort_order, $pagina){
        ...
        $dados['id'] = 'id';
        $dados['tabela'] = 'node';
        $dados['dados'] = $this->db->get(BANCO_TELECOM.'.tcom_node', $mostra_por_pagina, $pagina)->result();           
