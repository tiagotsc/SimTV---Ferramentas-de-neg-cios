...
class chart_model extends CI_Model {
		...
		foreach($r as $x)
		{
			if($x['area'] !== $l)
			{
				$result['series'][] = array('name'=> $l." - TV",'data'=>$c_tv,'color' =>
						(($l !== 'SIM')?
							$this->base_model->areas['nome'][$this->base_model->area_abbr($l)]['color']
						:
							'black')
						);
	...
	function backlogDPChart($b){
		$tv_color = '#002F8E';
		$cm_color = '#3260BC';
		$ars = $this->db->order_by('id')->get('area')->result_array();
		$r = array(
			'categories'=>array(),
			'series' => array(
				array(
		...
		foreach($areas as $area){
			$r['categories'][] = $this->base_model->area_abbr($area['name']);
			for($i = 0;$i<3;$i++)
	...
	function backlogChart($b){
		...
		foreach($b as $x){
			$a = $this->base_model->area_abbr($x['area']);
			$c = intval($x['c']);
			if($hascorp)
				$corp = intval($x['corp']);
	...
	function imbChart($month=false,$l = false){
		...
		$areas = $this->db->select('id,name')->order_by('id')->get('area')->result_array();
		$imb = array(array('reclamacoes'=>0,'base'=>0,'meta' => $this->base_model->sim_imb_meta,'name'=>'SIM','color' => $this->sim_tv_color));

		foreach($areas as $area){
			$aid = intval($area['id']);
			$x = $this->base_model->imb($aid,$month);
			$x['name'] = $area['name'];
		...
		$result['categories'] = array();
		foreach($imb as $a){
			$result['categories'][] = $this->base_model->area_abbr($a['name']);
		}
		...
		$result['series'][] = array('name' => 'Meta','type' => 'spline',
	...
	function imbMetaHist($cid,$m){
		...
		$result = array();
		$cm = $this->base_model->imb($cid,$m,'cm');
		$tv = $this->base_model->imb($cid,$m,'tv');
		$total = $this->base_model->imb($cid,$m);
		$result['total'] = $total;
		$result['tv'] = $tv;
		$result['cm'] = $cm;
		if(!$this->base_model->mes_fechado($m)){
	...
	function imbGroupsChart($month,$lmonth){
			...
			foreach($areas as $a){
				...
				foreach(array('tv','cm') as $p){
					$x = $this->base_model->imb($a['name'],$month,$p);
					if($lmonth)
	...
	function imbHistChart($lines,$sim){
			foreach($sim as $line){$lines[] = $line;}
			$areas = array();
			$c = 0;
			$m = array();
			foreach($lines as $line){
					$a = $this->base_model->area_abbr($line['aname']);
					$ptmes = ptmes($line['rmes']);
					if(!array_key_exists($a,$areas)){
									$areas[$this->base_model->area_abbr($line['aname'])] = $c;
									if($line['aname'] == 'SIM')
											$series[] = array('name' =>  $a,'data' => array(),'lineWidth' => 5,'color' => 'black');
									else
											$series[] = array('name' =>  $a,'data' => array(),'color'=>$this->base_model->areas['nome'][$this->base_model->area_abbr($line['aname'])]['color']);
									$c++;
					}
					if(!in_array($ptmes,$m))
						$m[] = $ptmes;
					...
					}elseif(!$this->base_model->mes_fechado($line['rmes'])){
	...
	function timesInfinity($manut){
		...
		$lday = intval(date('j',strtotime($this->base_model->lday)));
		$ldaym = intval(date('t',  strtotime($this->base_model->currmonth)));
		if($lday)
	...
	function revisitaPorMes($mes=false){
		...
		$rev['data'][] =
			array(
				...
				'name'=>'SIM',
				'color' => '#A4A4A4',
				'meta' => $this->base_model->sim_irm_meta
			);
		$mt = array('data'=>array());
		$mt['name'] = 'Meta IRM';
		$mt['type'] = 'spline';
		...
		$areas = $this->db->order_by('name')->get("area")->result_array();
		foreach($areas as $x){
			$a = $this->base_model->revisita($mes,$x['id']);
			$a_m = $this->base_model->mmeta('meta_irm',$a['name'],$mes);
	...
	function tecProdArea($tec,$mes){
		...
		if($tec)
			foreach($tec as $i => $t){
				unset($tec[$i]['ainfo']);
				if(!(in_array($t['name'],$this->base_model->nontecs) || !$t['name'])){
					if($c === 0){
						if($mes)
							$time = date('Y-m',strtotime($mes)).'-'.date('t',strtotime($mes));
						else
							$time = date('Y-m-d');
						$a = $this->base_model->busca_area_metas($t['item']['area'],$time);
	...
	//exit(json_encode($tec));
		$areas = array();
		$meta = array('x'=>$this->base_model->sim_meta_qualidade,'y'=>$this->base_model->sim_meta_producao);
		$range =
			array(
				...
				array(
					'x' =>
						(($areas[$i]['data'][0]['vts'] > 0)?
							round((1 - ( $areas[$i]['data'][0]['xvts']/$areas[$i]['data'][0]['vts'] ) )* 100,2)
							:0
						),
					'y' =>
						(($areas[$i]['data'][0]['ds'] > 0)
							?round($areas[$i]['data'][0]['vts']/$areas[$i]['data'][0]['ds'],2)
							:0
						),
					'ds' =>
						(($areas[$i]['data'][0]['tecs'] > 0)
							?round($areas[$i]['data'][0]['ds']/$areas[$i]['data'][0]['tecs'],2)
							:0
						)
				);
	...
	function vendInst($x){
		...
		foreach($x as $y){
			...
			if( !array_key_exists($aid,$areas) ){
				...
				$result['categories'][] = $this->base_model->area_abbr($y['area']);
			}
	...
	function st_realtime($st){
		...
		if($st === 'P'){
			$l['corp'] = $this->base_model->backlog_pendente(true);
            //$x[0]['corp'] = count($l['corp']);
			$l['ind'] = $this->base_model->backlog_pendente(false);
			//$x[0]['y'] = count($l['corp']) + count($l['ind']);


        }else{
            $l['corp'] = $this->base_model->backlog_status($st,true);
			//$x[0]['corp'] = count($l['corp']);
			$l['ind'] = $this->base_model->backlog_status($st,false);
			//$x[0]['y'] = count($l['corp']) + count($l['ind']);
        }
		$p = $this->db->
			...
			if($ordemAge !== null){
				...
				$series[$ordemAge]['os'][] = 
					array(
						...
						'agenda' => 
							(($ordem['min_ini'])
								?date('d/m/Y',strtotime(substr($ordem['min_ini'], 0, 10)))
								.' - '.
	...
	function cst_meta_val($meta,$area = null,$time = null){
		...
		if($time)
			$this->db->where('ini <=', ( ( strlen($time)>= 10 ) ? $time : $time.'-01' ) );

		$x = $this->db->order_by('ini DESC, id DESC')->limit(1)->get('cst_meta')->row_array();
	...
	function cst_cluster_meta_val($meta,$area = null,$time = null){
		...
		$this->db->where('meta',$meta);
		$this->db->where('cst_cluster',$area);
		
		if($time)
			$this->db->where('ini <=', ( ( strlen($time)>= 10 ) ? $time : $time.'-01' ) );

		$x = $this->db->order_by('ini DESC, id DESC')->limit(1)->get('cst_cluster_meta')->row_array();
		return (($x)?floatval($x['value']):null);
	}
}
?>
