                        ...
                        <table class="table table-bordered">
                            ...
                            <tfoot>
                                <tr>
                                    ...
                                    <td id="totalQtd">0</td>
                                    <td id="totalValorInst">R$ 0,00</td>
                            ...
                            <tbody id="tabelaConteudo">
                                <tr>
                                    <td colspan="7">Informe os parâmetros</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
...
$("#tipo_acao").change(function(){
   ...
   if($(this).val() != '' && $(this).val() != 'faturamento'){
    ...
    $("#dataDiv").hide();
    $("#tabelaConteudo").html('<tr><td colspan="7">Informe os parâmetros</td></tr>');
   }
   $(this).btnSubmit();
});
...
$.fn.carregaTabela = function(){ 
    ...
    $.ajax({
      ...
      error: function(res) { //dump(res);
        $("#tabelaConteudo").html('<tr><td colspan="7">Erro...</td></tr>');
      },
      success: function(res) {
        ...
        if(res.length > 0){
            ...
            $.each(res, function() {
                ...
                if(this.valor_cobrado_inst === null){
                    var valorInst = '0,00';
                }else{
                    var valorInst = this.valor_cobrado_inst;
                }             
                
                if( (this.faturado > 0) || (this.faturado > 0 && this.existe != '-' && this.existe == 0 )){
                ...
                if(this.arquivo === null || this.arquivo === undefined){
                    ...
                    var ico = '<span class="glyphicon glyphicon-info-sign" aria-hidden="true"></span>';
                    var arq = 'Delin corrente -> '+this.arquivo;
                }
                ...
                conteudo += '<td title="'+arq+'">'+ico+this.grupo+'</td>';
                conteudo += '<td class="qtd'+this.idPai+'">'+this.qtdOper+'</td>';
                conteudo += '<td class="valorInst'+this.idPai+'">R$ '+mascaraValor(valorInst)+'</td>';
            ...
            conteudo += '<tr><td colspan="7">Nada encontrado.</td></tr>';
        }
...
function calculaValores(id){
    ...
    var valorTotalInst = Number($("#totalValorInst").html().replace("R$ ", "").replace(".", "").replace(".", "").replace(",", "."));
    var qtdTotal = parseInt($("#totalQtd").html());
    var pegaValor = Number($(".valor"+id).html().replace("R$ ", "").replace(".", "").replace(".", "").replace(",", "."));
    var pegaValorInst = Number($(".valorInst"+id).html().replace("R$ ", "").replace(".", "").replace(".", "").replace(",", "."));
    ...
    if($("#grupo"+id).prop('checked') == true){
        ...
        $("#totalValorInst").html("R$ "+ mascaraValor((valorTotalInst+pegaValorInst).toFixed(2)) );
    }else{
        $("#totalQtd").html(qtdTotal-pegaQtd);
        $("#totalValor").html("R$ "+ mascaraValor((valorTotal-pegaValor).toFixed(2)) );
        $("#totalValorInst").html("R$ "+ mascaraValor((valorTotalInst-pegaValorInst).toFixed(2)) );
